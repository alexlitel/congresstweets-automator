...
import { getTime, nativeClone } from '../src/util'
import { mockChanges } from './util/test-util'
import { BuildMd, ChangeMessage } from '../src/helpers'
  ...
  describe('Class methods', () => {
    ...
    describe('wrapChangeData', () => {
      test('Wraps change data to lines of 72 characters max', () => {
        ...
        expect(
          ChangeMessage.wrapChangeData(
            Array.from(Array(8)).map((x, i) => `member ${i}`),
            'Changed:\n'
          )
    ...
    describe('flattenChanges', () => {
      test('Flattens, sorts, and replaces keys', () => {
        const changesBuild = Object.assign(mockChanges(true), {
          list: { add: [1], remove: [1] },
        })
        ...
        expect(flattened.map((x) => x[0])).toEqual([
          'accounts add',
          'accounts remove',
        ])
        expect(flattened2).toHaveLength(8)
        expect(flattened2.map((x) => x[0])).toEqual([
          'members add',
          'members remove',
          'members update',
          'accounts add',
      ...
      test('Omits keys and deleted accounts from removed members for commit', () => {
        ...
        expect(
          flattened.find((x) => x[0] === 'accounts delete')[1]
        ).toHaveLength(2)
      })
    ...
    describe('stringifyChangeList', () => {
      test('Stringifies list post-build', () => {
        ...
        const changeString =
          '\n\n1 account added\nTwitter\n\n2 accounts removed\nRemoved Twitter\nRemoved Twitter'
        expect(
          ChangeMessage.stringifyChangeList(changes, { postBuild: true })
      ...
      test('Stringifies list not in post-build', () => {
        const changes = [
          [
            'members add',
            [{ name: 'Person 2' }, { name: 'Person 1' }, { name: 'Person 3' }],
          ],
          [
            'accounts add',
            [
          ...
          [
            ...
            [
              {
                name: 'Person 9',
                screen_name: 'InactiveTwitterPerson',
                account_type: 'office',
              },
            ],
          ],
        ...
        const changeString = [
          ...
          'Accounts deactivated:\nInactiveTwitterPerson (Person 9 office)',
        ].join('\n\n')
    ...
    describe('createCommitMessage', () => {
      test('Converts change list with updated records to orderly short-ish commit message', () => {
        ...
        expect(ChangeMessage.createCommitMessage(changes)).toEqual(
          'Add Person & Person 2, remove Person, & update records'
        )
      })
      test('Converts change list sans updated records to orderly short-ish commit message', () => {
        ...
        expect(ChangeMessage.createCommitMessage(changes)).toEqual(
          'Add Person, remove Person, & add Person office account'
        )
      })
    })
    ...
    describe('summarizeChanges', () => {
      test('Post-build', () => {
        expect(
          ChangeMessage.summarizeChanges({}, {}, { postBuild: true })
        ).toEqual('Successful build')
      })
      test('Post build with store update', () => {
        expect(
          ChangeMessage.summarizeChanges(
            {},
            { storeUpdate: true },
            { postBuild: true }
          )
      ...
      test('New congress (more than 10 new/removed members) and isCommit flag enabled', () => {
        ...
        const changeMsg = ChangeMessage.summarizeChanges({}, changes, {
          isCommit: true,
        })
      ...
      test('Change count is greater than or equal to 10 and isCommit flag enabled', () => {
        const changes = {}
        changes.count = 11
        const changeMsg = ChangeMessage.summarizeChanges(
          changes,
          {},
          { isCommit: true }
        )
      ...
      test('No changes and/or local development environment', () => {
        expect(
          ChangeMessage.summarizeChanges({}, {}, { isProd: true })
        ).toEqual('Successful server maintenance process')
        expect(
          ChangeMessage.summarizeChanges({}, {}, { isProd: false })
        ).toEqual('Successful local maintenance process')
      })
    })
    ...
    describe('create (change message generation)', () => {
      describe('No changes', () => {
        test('Post-build', () => {
          ...
          expect(ChangeMessage.create(changes, { postBuild: true })).toEqual(
            'Successful build'
          )
        })
        test('Production env', () => {
          ...
          expect(ChangeMessage.create(changes, { isProd: true })).toEqual(
            message
          )
        })
        test('Local env', () => {
          ...
          expect(ChangeMessage.create(changes, { isProd: false })).toEqual(
            message
          )
        })
      })
      ...
      describe('Changes in production env', () => {
        ...
        test('Prints serialized change list for commit', () => {
          ...
          changes.list.deleted.push(
            Object.assign({}, mockAccount[0], {
              bioguide: '222',
              name: 'member 5',
            })
          )
          ...
          expect(
            changeMsg.startsWith(
              'Add member 2, remove member 1, add member 7 office account, delete member 5 office account, & update records'
            )
          ).toBeTruthy()
